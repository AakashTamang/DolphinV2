...
def calculate_progress(designation_dates):
    ...
    normailized_design = ['Junior Employee','Intermediate Employee','Employee','Senior Employee']
...
def one_resume_multiple_jd_scorer(job,designations,user_exp,user_skills,user_location, designation_dates):
    ...
    try:
        skill_score = (len(matched_soft_skills)+(len(matched_technical_skills)))/(len(req_technical_skills)+len(req_soft_skills)) *25
    except:
        skill_score = 25
...
def one_JD_multiple_resume_scorer(profile,job_title,req_exp,req_soft_skills,req_technical_skills,employer_city):
    ...
    matched_technical_skills = set(user_skills).intersection(set(req_technical_skills))
