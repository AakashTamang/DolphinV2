...
def oneResMultipleJD():
    ...
    user_skills,user_exp,designations,user_location,designation_dates = prepare_profile(user_profile)
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        result = [executor.submit(one_resume_multiple_jd_scorer,job_description,designations,user_exp,user_skills,user_location, designation_dates) for job_description in job_descriptions]
    my_score = {}
...
def oneJDMultipleRes():
    ...
    #Multiprocessing because of heavy computational time
    with concurrent.futures.ProcessPoolExecutor() as executor:
        results = [executor.submit(one_JD_multiple_resume_scorer,profile,job_title,req_experience,req_soft_skills,req_technical_skills,employer_location) for profile in user_profiles]
    my_score = {}
    
    for f in concurrent.futures.as_completed(results):
        id,total_score = f.result()
        my_score[id] = total_score
    result = {job_id:my_score}
    return result

@app.route("/",methods=['GET'])
def check_site():
    return ('api is live! Thank you')

if __name__ == "__main__":
    app.run(host='127.0.0.2', port=8002, debug=True)